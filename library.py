...
Использование:
    ...
    # Кастомный обработчик (переопределение ответа):
    from aiohttp import web
    from library import EndpointConfig

    async def my_handler(request, config, target_url, method, body, headers, params):
        # Возвращаем свой ответ вместо запроса к API
...
class EndpointConfig:
    ...
    description: str = ""
    custom_handler: Callable | None = None  # Пользовательская функция обработки ответа
    use_custom_handler: bool = False  # Флаг использования пользовательского обработчика
...
class EndpointRoute:
    ...
    # Переопределения, установленные через set_custom_handler
    custom_handler_override: Callable | None = None
    use_custom_handler_override: bool | None = None
...
class ProxyRouter:
    ...
    def add_endpoint(
        ...
        route = EndpointRoute(prefix=path, config_factory=None)  # сначала создаём объект
        ...
        def factory() -> EndpointConfig:
            cfg = EndpointConfig(
                ...
                use_custom_handler=use_custom_handler,
            )
            # Применяем оверрайды с этого route-объекта
            if route.custom_handler_override is not None:
                cfg.custom_handler = route.custom_handler_override
            if route.use_custom_handler_override is not None:
                cfg.use_custom_handler = route.use_custom_handler_override
            return cfg

        route.config_factory = factory
    ...
    # ------------------------------------------------------------------
    # Динамическая установка кастомных обработчиков
    # ------------------------------------------------------------------
    ...
    def set_custom_handler(
        ...
        """Устанавливает кастомный обработчик для эндпоинта по пути.

        Args:
            path: Путь эндпоинта (например, "/v3")
            handler: Асинхронная функция-обработчик или None для отключения
            enabled: Включить обработчик (True) или просто сохранить функцию

        Returns:
            True если обработчик найден и обновлён, False если путь не найден
        """
        for route in self._endpoints:
            if route.prefix == path:
                route.custom_handler_override = handler
                route.use_custom_handler_override = enabled
                log.info("Custom handler %s for %s",
                         "enabled" if enabled else "saved", path)
                return True

        log.warning("Endpoint path '%s' not found for custom handler", path)
        return False
